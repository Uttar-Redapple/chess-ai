...
// Constants
const FRICTION = gameConfig.FRICTION;
// const FRICTION = 0;
...
physics.collideCushions = function (ball, width, height) {
    ...
    // Check if about to collide with cushion
    if (ball.position.x + ball.velocity.x + ball.radius >= (width/2) || ball.position.x + ball.velocity.x - ball.radius <= (-(width/2))) {
        // Reverse velocity
    ...
    if (ball.position.y + ball.velocity.y + ball.radius >= (height/2) || ball.position.y + ball.velocity.y - ball.radius <= (-height/2)) {
        // Reverse velocity
